...
结果表会优先按 System Prompt 中声明的顶层 JSON 变量分列，同时补充模型实际返回的额外字段。纯文本输出则显示为单独的“模型输出”列。Excel 和 CSV 导出会完整保留原始 Excel 的所有列、Markdown 文本和单元格内换行，再在右侧追加评估状态和模型输出；重名的评估字段会自动加 `评估_` 前缀。
