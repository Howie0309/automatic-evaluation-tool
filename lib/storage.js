...
import { parseJudgeOutput } from './evaluator.js';
import { parseWorksheet } from './excel.js';
import { buildExportRows, deriveOutputColumns } from '../public/result-export.js';
...
export function buildResultsCsv(results, outputColumns, inputColumns = [], config = {}) {
  ...
  const effectiveOutputColumns = deriveOutputColumns(results, outputColumns);
  const { rows } = buildExportRows(results, effectiveOutputColumns, inputColumns, config);
  return '\ufeff' + rows.map(row => row.map(quote).join(',')).join('\r\n');
}

export async function buildResultsXlsx(results, outputColumns, inputColumns = [], config = {}) {
  const effectiveOutputColumns = deriveOutputColumns(results, outputColumns);
  const { schema, rows } = buildExportRows(results, effectiveOutputColumns, inputColumns, config);
  ...
  const compactLabels = new Set([
    ...
    schema.labels.attempts, schema.labels.elapsed, schema.webSearch?.status
  ]);
  worksheet.columns.forEach((column, index) => {
    ...
    column.alignment = {
      ...
      wrapText: !compactLabels.has(label)
    };
  });
...
function safeConfig(config = {}) {
  const allowed = [
    ...
    'reasoningEffort', 'webSearchMode', 'scorePath', 'retryCount', 'concurrency', 'queryColumn', 'answerColumn', 'promptMappings'
  ];
...
export async function readRunArtifact(id, format, root = defaultStorageRoot) {
  ...
  const record = normalizeRunOutputs(await hydrateLegacyRecord(JSON.parse(jsonContent.toString('utf8')), root));
  if (format === 'json') {
...
export function normalizeRunOutputs(record) {
  let changed = false;
  const results = (record.results || []).map(item => {
    if (typeof item.output !== 'string') return item;
    const output = parseJudgeOutput(item.output);
  ...
  return {
    ...record,
    results,
    outputColumns: deriveOutputColumns(results, record.outputColumns || [])
  };
}
